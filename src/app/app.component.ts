...
ClientService
const addPath = (urlAndQuery: string[]) =>
  urlAndQuery[0] ? '/' + urlAndQuery[0] : '';
const addQuery = (urlAndQuery: string[]) =>
 ...
 router.events.forEach((event) => {
  ...
  private routeCheck(event): void {
    if (this.tenants.length === 0) {
      this.clientService.getClients({ status: true }).subscribe(
        (data) => {
          if (data?.length > 0) {
            this.tenants = data;
  ...
  private routingGuard(event): void {
    ...
    if (
      this.tenants.findIndex(
        (tenant) =>
          tenant.clientName.toLowerCase() === firstPathPart.toLowerCase()
      ) !== -1 ||
      ...
      sessionStorage.setItem(
        'tenant-id',
        this.tenants.find(
          (t) => t.clientName.toLowerCase() === firstPathPart.toLowerCase()
        ).id
      );
    } else {
      ...
      if (this.activeTenant) {
        ...
        this.router.navigate([`/${firstPathPart}/error`], {
      ...
      const redirectUrl =
        '/' + prefix + addPath(urlAndQuery) + addQuery(urlAndQuery);
